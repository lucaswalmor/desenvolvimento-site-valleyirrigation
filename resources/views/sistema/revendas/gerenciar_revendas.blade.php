    ...
    <div class="container-fluid topo">
        <div class="row align-items-start">
            ...
            <div class="col-6 text-right mobile">
                <a href="{{ route('create_resales') }}" data-toggle="tooltip" data-placement="left"
        ...
        <div class="row justify-content-end telo5inputfiltro mt-3">
            <div class="col-md-3 position">
                <form action="{{route('filter_resales')}}" method="POST" class="form form-inline">
                    @csrf
        ...
        <table class="table table-striped mx-auto" id="filtertable">
            ...
            <thead class="headertable">
                <tr class="text-center">
                    <th scope="col-5">@lang('revendas.nome')</th>
                    <th scope="col-5">@lang('revendas.telefone')</th>
                    <th scope="col-5">@lang('revendas.email')</th>
                    <th scope="col-2">@lang('sidenav.acoes')</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($manager_revenda as $revenda)
                    <tr>
                        <td>{{ $revenda->nome }}</td>
                        <td>{{ $revenda->telefone }}</td>
                        <td>{{ $revenda->email }}</td>
                        <td class="acoes">
                            <a href="{{ route('edit_resales', $revenda->id) }}"><button type="button"
                                    class="botaoTabela"><i class='fa fa-fw fa-pen'></i></button></a>
                            <button type="submit" class="botaoTabela" data-toggle="modal"
                                data-target="#modalDeletar-{{ $revenda['id'] }}"><i
                                    class='fa fa-fw fa-times'></i></button>

                            <div class="modal fade" id="modalDeletar-{{ $revenda['id'] }}" tabindex="-1"
                                aria-labelledby="modalDeletar" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h4 style="margin: 0">@lang('comum.revendas') {{ $revenda['nome'] }}</h4>
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form
                                                action="{{ action('Sistema\RevendasController@delete', $revenda['id']) }}"
